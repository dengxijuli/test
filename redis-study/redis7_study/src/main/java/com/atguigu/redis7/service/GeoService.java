...
public class GeoService {
    ...
    public GeoResults radiusByMember() {
        //通过地方查找附近
//        new GeoReference<String>();
        Distance distance = new Distance(200, Metrics.KILOMETERS);//params: 距离量, 距离单位
        RedisGeoCommands.GeoRadiusCommandArgs args = RedisGeoCommands.GeoRadiusCommandArgs.newGeoRadiusArgs().includeDistance().includeCoordinates().sortAscending().limit(5);
        GeoResults<RedisGeoCommands.GeoLocation<String>> results = redisTemplate.opsForGeo()
                .radius(CITY, "故宫", distance, args);//params: key, 地方名称, Circle, GeoRadiusCommandArgs

        RedisGeoCommands.GeoSearchCommandArgs args1 = RedisGeoCommands.GeoSearchCommandArgs.newGeoSearchArgs().includeDistance().includeCoordinates().sortDescending().limit(50);
        GeoResults<RedisGeoCommands.GeoLocation<String>> 故宫 = redisTemplate.opsForGeo().search(CITY, new GeoReference.GeoMemberReference("故宫"), new Distance(200, Metrics.KILOMETERS), args1);
